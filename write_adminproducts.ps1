...
export default function AdminProducts() {
  ...
  const handleSubmit = (e) => {
    ...
    const { image, newArrival, promotion, bestSeller, featured, oldPrice, ...rest } = form;
    const payload = {
      ...rest,
      images: image ? [image] : [],
            ...
            <tbody>
              {products.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>
                    {p.stock > 0 ? (
                      <span className="admin-badge admin-badge--delivered">{p.stock} en stock</span>
                    ) : (
            ...
            <form className="admin-modal-form" onSubmit={handleSubmit}>
              ...
              <div className="admin-modal-form__row">
                <div>
                  ...
                  <input id="pf-stock" name="stock" type="number" placeholder="0" value={form.stock} onChange={handleChange} />
                </div>
                <div>
...
'@
Set-Content -Path "src\pages\admin\AdminProducts.jsx" -Value $content -Encoding UTF8
